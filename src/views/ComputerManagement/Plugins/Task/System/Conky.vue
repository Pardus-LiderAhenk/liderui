    ...
    >
      ...
      <template #default>
        <div class="p-grid  p-flex-column">
          ...
          <div class="p-col">
            <TabView class="tabview-custom" ref="tabview4" @tab-click="asd" v-model:activeIndex="activeIndex">
              <TabPanel>
                <template #header>
            ...
            <Textarea v-if="selectedTemplate && activeIndex === 0" v-model="selectedTemplate.contents"  style="height: 320px; width:100%" />
            <Textarea v-if="selectedTemplate && activeIndex === 1" v-model="selectedTemplate.settings"  style="height: 320px; width:100%" />
            <p v-if="!selectedTemplate" class="p-text-center">{{$t('computer.plugins.conky.warn_select_template')}}</p>
          </div>
        </div>
      </template>
...
export default {
  ...
  mounted() {
    ...
    axios.post(process.env.VUE_APP_URL + "/conky/list", params)
    .then((response) => {
    ...
    .catch((error) => { 
      this.$toast.add({severity:'error', detail: this.$t('computer.plugins.conky.conky_error_message')+ " \n"+error, summary:this.$t("computer.task.toast_summary"), life: 3000})})
  },
  ...
  methods: {
    sendTaskConky(removeConky){
      ...
      this.task.parameterMap = {
        "conkyMessage": this.selectedTemplate ? this.selectedTemplate.settings + "\n" + this.selectedTemplate.contents: "",
